...
def delete_folder(swarm: SwarmConfig, folder_path: str) -> dict:
    try:
        blob_service_client = BlobServiceClient(
            account_url=f"https://{swarm.azure_blob_storage_account_name}.blob.core.windows.net/",
            credential=swarm.azure_blob_storage_account_key
        )
        container_client = blob_service_client.get_container_client(swarm.azure_blob_storage_container_name)
...
def make_folder(swarm: SwarmConfig, folder_path: str) -> None:
    return None

def move_folder(swarm: SwarmConfig, folder_path: str, new_folder_name: str) -> None:
    try:
        rename_folder(swarm, folder_path, new_folder_name)
    except Exception as e:
        raise ValueError(f'Failed to move folder: {str(e)}')

def rename_folder(swarm: SwarmConfig, folder_path: str, new_folder_path: str) -> None:
    try:
        blob_service_client = BlobServiceClient(
            account_url=f"https://{swarm.azure_blob_storage_account_name}.blob.core.windows.net/",
            credential=swarm.azure_blob_storage_account_key

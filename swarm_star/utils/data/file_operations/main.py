...
from swarm_star.utils.data.paths import validate_and_adjust_swarm_space_path

if TYPE_CHECKING:
    from swarm_star.swarm.types import SwarmConfig

platform_map = {
    'mac': 'swarm_star.utils.data.file_operations.local_storage',
    'azure': 'swarm_star.utils.data.file_operations.azure_blob_storage',
}
...
def move_swarm_space_file(swarm: SwarmConfig, file_path: str, new_file_path: str) -> None:
    platform = swarm.platform
    file_path = validate_and_adjust_swarm_space_path(file_path, swarm.swarm_space_root_path)
...
def rename_swarm_space_file(swarm: SwarmConfig, file_path: str, new_file_name: str) -> None:
    platform = swarm.platform
    file_path = validate_and_adjust_swarm_space_path(file_path, swarm.swarm_space_root_path)
...
def upload_swarm_space_file(swarm: SwarmConfig, file_path: str, file_bytes: bytes) -> None:
    platform = swarm.platform
    file_path = validate_and_adjust_swarm_space_path(file_path, swarm.swarm_space_root_path)
    file_operations_module = import_module(platform_map[platform])

...
from swarm_star.swarm.types import SwarmConfig, TerminationOperation, SwarmState, SpawnOperation, NodeEmbryo
...
def execute_termination_operation(swarm: SwarmConfig, termination_operation: TerminationOperation) -> Union[TerminationOperation, None]:
    ...
    for child_id in node.children:
        ...
        if child.alive:
            return None
        if child.action_id == 'swarm_star/actions/reasoning/confirm_completion':
            mission_completion = True
        if child.action_id == 'swarm_star/actions/reasoning/consolidate_reports':
    ...
    if mission_completion == False:
        return SpawnOperation(
            ...
            node_embryos=[
                NodeEmbryo(
                    node_id=node_id,
                    action_id='swarm_star/actions/reasoning/confirm_completion',
                    message=''
                )    
            ],
        )
        ...
        if reports_consolidated == False:
            return SpawnOperation(
                ...
                node_embryos=[
                    NodeEmbryo(
                        node_id=node_id,
                        action_id='swarm_star/actions/reasoning/consolidate_reports',
                        message=''
                    )
                ],
            )

...
with open('z/gratamatta/swarm_space.json', 'r') as f:
    swarm_space_dict = json.loads(f.read())

swarm_space = SwarmSpace.model_validate(swarm_space_dict)
...
manager_node = _spawn_node(swarm_space, swarm_command, None)



nodes: List[SwarmNode] = swarm_master(swarm_space, manager_node)
print('\n\n\n')
for node in nodes:

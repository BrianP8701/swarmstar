...
from swarmstar.swarm.types import SpawnOperation, NodeEmbryo, BlockingOperation
...
def test_ask_user_questions():
    ...
    while next_swarm_operation[0].operation_type != 'spawn' and next_swarm_operation[0].operation_type != 'terminate':
        if next_swarm_operation[0].blocking_type == 'instructor_completion' or next_swarm_operation[0].blocking_type == 'internal_action':
            ...
            user_input = input()
            next_swarm_operation = [BlockingOperation(
